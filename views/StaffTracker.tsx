        ...
        {filteredStaff.length > 0 ? filteredStaff.map(staff => (
          <div 
            ...
            className={`group bg-white rounded-[3rem] p-8 border transition-all duration-300 relative overflow-hidden ${
              staff.status === 'Active' ? 'border-emerald-100 shadow-xl shadow-emerald-500/5' :
          ...
          >
            <div className="flex items-center gap-6 mb-8">
              <div className="relative">
                <div className={`w-20 h-20 rounded-3xl flex items-center justify-center text-2xl font-black transition-colors ${
                  staff.status === 'Active' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/20' :
              ...
              {staff.status === 'Out' ? (
                ...
                <>
                  <button 
                    ...
                    className={`flex flex-col items-center justify-center gap-2 py-4 rounded-2xl font-bold text-[10px] transition-all border-2 ${
                      staff.status === 'Active' 
                ...
                <div className={`flex justify-center gap-4 py-4 ${isError ? 'animate-shake' : ''}`}>
                   {[0,1,2,3].map(i => (
                     <div key={i} className={`w-4 h-4 rounded-full transition-all duration-200 border-2 ${
                       i < pinBuffer.length ? 'bg-blue-600 border-blue-600 scale-125' : 'bg-transparent border-slate-200'
                     }`}></div>
                   ))}
                </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                   {['1','2','3','4','5','6','7','8','9','C','0','DEL'].map((val) => (
                     <button
                        ...
                        className={`h-20 rounded-[2rem] text-2xl font-black transition-all active:scale-90 ${
                          val === 'C' ? 'bg-slate-100 text-slate-400 text-lg' :
